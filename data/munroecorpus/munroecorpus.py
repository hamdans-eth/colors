...
def get_training_handles():
	...
	return [{name:curdir+"/train/%s.%s"%(fn,ext) for name,fn in list(c_index.items())} for ext in exts]
...
def get_dev_handles():
	...
	curdir = os.path.dirname(os.path.abspath(__file__))
	fp = open(curdir+"/corpusindex.txt"); c_index = {x.replace("\n","").split(",")[0]:x.replace("\n","").split(",")[1] for x in fp.readlines()}; fp.close()
	return {name:curdir+"/5dev/%s.test"%(fn) for name,fn in list(c_index.items())} 	
...
def get_test_handles():
	...
	curdir = os.path.dirname(os.path.abspath(__file__))
	fp = open(curdir+"/corpusindex.txt"); c_index = {x.replace("\n","").split(",")[0]:x.replace("\n","").split(",")[1] for x in fp.readlines()}; fp.close()
	return {name:curdir+"/25test/%s.test"%(fn) for name,fn in list(c_index.items())} 	
