...
encoder = rnn5.EncoderRNN(vocabulary.n_words, rgb_dim_n,embeddings).to(device)
decoder = rnn5.DecoderRNN(embedding_dim_n,embeddings,vocabulary.n_words).to(device)
linear = rnn5.RGB_to_Hidden(rgb_dim_n, embedding_dim_n).to(device)

trainIters(encoder, decoder,linear, epochs,plot_every=500,  print_every=500, learning_rate = lr)


import os
dirpath = os.getcwd()
encoder_path = dirpath + '/enc_simple'
decoder_path = dirpath + '/dec_simple'
torch.save(encoder, encoder_path)
torch.save(decoder, decoder_path)
lin_path = dirpath + '/lin_simple'
